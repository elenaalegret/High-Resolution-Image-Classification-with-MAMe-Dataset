...
def train_perceptron():
    ...
    # 256 x 256
    model.add(
        Conv2D(32, (3, 3), padding='same', activation='relu', input_shape=input_shape, kernel_initializer=he_normal(), kernel_regularizer=regularizers.l1(1e-5),
               strides=2))  # [B, 128, 128, 32]
    # 128 x 128
    model.add(Conv2D(32, (3, 3), padding='same', activation='relu', kernel_regularizer=regularizers.l1(1e-5),
                     kernel_initializer=he_normal(), strides=2)) # [B, 64, 64, 32]
    model.add(MaxPooling2D(pool_size=2)) # [B, 32, 32, 32]
    # 32 x 32
    model.add(Conv2D(64, (3, 3), padding='same', activation='relu', kernel_regularizer=regularizers.l1(1e-5),
                     kernel_initializer=he_normal()))
    model.add(MaxPooling2D(pool_size=2)) # [B, 16, 16, 32]
    # 16 x 16
    model.add(Conv2D(128, (3, 3), padding='same', activation='relu', kernel_regularizer=regularizers.l1(1e-5),
                     kernel_initializer=he_normal()))
    model.add(MaxPooling2D(pool_size=2)) # [B, 8, 8, 32]
    # 8 x 8
    model.add(Conv2D(128, (3, 3), padding='same', activation='relu', kernel_regularizer=regularizers.l1(1e-5),
                     kernel_initializer=he_normal()))
    model.add(MaxPooling2D(pool_size=2)) # [B, 4, 4, 32]
    # 4 x 4
    model.add(Conv2D(128, (3, 3), padding='same', activation='relu', kernel_regularizer=regularizers.l1(1e-5),
                     kernel_initializer=he_normal()))
    # Flat:
